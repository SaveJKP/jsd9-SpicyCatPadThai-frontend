  ...
  return (
    <div className="flex h-[64px] flex-row justify-between bg-[var(--color-background)] px-[24px] py-1 text-[var(--color-text)] sm:max-md:px-0">
      <div class="flex flex-row">
        ...
        <Link to="/" class="flex flex-row items-center sm:max-md:hidden">
          <img src={logo_katsubook_notext} alt="onlyLogo" class="h-[70%]" />
          <img src={logo_katsubook_text} alt="onlyLogoText" class="h-[60%]" />
        </Link>
      ...
      <div class="flex flex-row justify-end gap-5">
        {!token ? (
          <>
            ...
            <Link
              to={handleLogout}
              class="my-2 block w-[30%] justify-self-center rounded-xl bg-[var(--color-buttonBrown)] p-[8px] text-center text-[16px] hover:bg-[#bc7142cb] min-[1024px]:block min-sm:hidden"
            >
              Log out
            </Link>
          </>
