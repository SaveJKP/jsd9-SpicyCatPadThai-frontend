...
import Autoplay from 'embla-carousel-autoplay'
  ...
  export default function BannerSlider() {
    ...
    return (
    <div className="flex justify-center items-center w-full h-fit bg-banner">
        ...
        <CarouselContent>
            {bannerItems.map((item) => {
                return  <CarouselItem className="flex flex-row justify-center md:justify-between items-center w-[100%]" key={item.id}>
                <div className="relative z-10 flex flex-col md:w-1/3 px-6 py-8 lg:w-1/2 justify-center pl-12">
                ...
                <div className="absolute md:relative z-0 md:flex w-full md:w-2/3 lg:w-1/2">
                    <img
                        src={item.banner}
                        alt={item.title}
                        className="w-full h-auto object-cover"
                    />
                    <div className="absolute md:hidden inset-0 bg-black opacity-65"></div>
                </div>
                </CarouselItem>
            }
        )};
